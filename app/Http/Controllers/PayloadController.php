...
class PayloadController extends Controller
{
    public function index() : JsonResponse
    {
        LogService::add(
            array(
                "message" => "Email to sent with success",
                "timestamp" => time()
            )
            );
            dd();
        $payload = $this->IOTmachineSendData();
        $header = DataParser::parseHeader(substr($payload, 0, 14));
        $data = DataParser::parseData(substr($payload, 14));

        $response = $this->dispatch(new SendEmailJob("priccardo_sp@hotmail.com", "<h1>teste</h1>"));
        ...
        ->json([
            ...
            "header" => $header,
            "data" => $data,
            "email_queued" => $response 
        ]);
    }

    private function IOTmachineSendData() : string
    {
        $data = [
